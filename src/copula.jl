...
GaussianCopula(cov::Array{Float64,2}) = GaussianCopula(cov, transpose(cholesky(cov).U))
...
function Base.rand(rng::AbstractRNG, c::GaussianCopula)
    return normal_cdf(sample_mvn(rng, c._chol))
end
Base.rand(c::GaussianCopula) = rand(Base.GLOBAL_RNG, c)